...
export const defaultSiteImages: SiteImage[] = [
  { id: "logo", label: "Marketplace logo", description: "The primary logo used in the header and footer.", url: "/manus-storage/cannery-marketplace-logo-brand_e839fd29.png", alt: "The Cannery Marketplace — Gilroy, California", caption: "The Cannery Marketplace", recommended: "Transparent PNG or SVG", defaultUrl: "/manus-storage/cannery-marketplace-logo-brand_e839fd29.png" },
...
export function readSiteImages(): SiteImage[] {
  ...
  try { const saved = window.localStorage.getItem(siteImagesStorageKey); if (!saved) return defaultSiteImages; const parsed = JSON.parse(saved) as SiteImage[]; return parsed.map((image) => image.id === "logo" && image.url.includes("cannery-marketplace-logo_659f3936") ? { ...image, url: defaultSiteImages[0].url, defaultUrl: defaultSiteImages[0].defaultUrl } : image); } catch { return defaultSiteImages; }
}
