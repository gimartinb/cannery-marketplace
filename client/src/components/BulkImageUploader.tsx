...
function processImage(file: File, squareCrop: boolean): Promise<{ url: string; width: number; height: number }> {
  return new Promise((resolve, reject) => {
    ...
    source.onload = () => {
      ...
      const context = canvas.getContext("2d"); if (!context) return reject(new Error("Canvas unavailable"));
      ...
      resolve({ url: canvas.toDataURL("image/jpeg", .86), width: source.naturalWidth, height: source.naturalHeight });
    };
    source.onerror = () => reject(new Error("Could not read image"));
...
export default function BulkImageUploader() {
  ...
  async function inspect(files: FileList | File[]) {
    ...
    for (const file of Array.from(files)) {
      ...
      } catch { nextInvalid.push(`${file.name} · could not be processed`); }
    }
